...
def main():
    requests.packages.urllib3.disable_warnings(
        InsecureRequestWarning)  # ssl warning disable
    ...
    for row in table.find_all("tr"):
        ...
        if len(col) != 0 and col[4].get_text().strip() != 'EPJ':
            ...
            crsnames.append(
                col[2].get_text().strip() +
                '-' +
                col[3].get_text().strip())
    ...
    for i in range(0, len(classids)):
        cp = requests.post(
            'https://vtopbeta.vit.ac.in/vtop/getCourseForCoursePage',
            headers=newheader,
            data={
                'semSubId': semSubId,
        ...
        for key in rows1.keys():  # upper table parsing
            ...
            path = "CoursePageDownloads//" + \
                crsnames[i] + '//' + facnames[i] + \
                '-' + slots[i].replace('/', '+')
